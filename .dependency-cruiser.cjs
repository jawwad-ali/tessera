...
module.exports = {
  ...
  options: {
    ...
    // `next-env.d.ts` is generated by `next build`, gitignored, and references
    // `next/image-types/global`, which is a types-only path depcruise cannot resolve. It failed
    // CI once when a background build wrote it moments before a commit.
    exclude: { path: '(^|/)(node_modules|dist|\\.next|coverage|bench-out)/|(^|/)next-env\\.d\\.ts$' },
